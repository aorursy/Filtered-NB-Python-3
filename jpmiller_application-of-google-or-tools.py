...
def set_loads(assignments):
    ...
    assign_df = assign_df.reindex(assign_df.index.repeat(assign_df.quant))                          .reset_index(drop=True)                          .assign(quant=1,
...
def reset_loads(df_end_reordered):
    df_end_reordered = df_end_reordered.reset_index(drop=True)                                  .assign(weight = lambda x: weights[x.prod_.to_numpy()] * x.quant)                                  .groupby('wh', as_index=False).apply(load_drones_improved)
...
df_end_reordered = df_end.groupby('wh').apply(order_orders)                          .pipe(reset_loads)                          .pipe(assign_drones)
...
with open('submission.csv', 'w') as sub:
    ...
    for dt in drone_tag.itertuples():
        df_load_tag = df_end_loading[(df_end_loading.load_tag == dt.load_tag) &                                           (df_end_loading.drone_id == dt.drone_id)]
        df_deliver_tag = df_end_reordered[(df_end_reordered.load_tag == dt.load_tag) &                                           (df_end_reordered.drone_id == dt.drone_id)]
