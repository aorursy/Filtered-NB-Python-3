...
n_hidden_3 = mlp_layer_sizes[3]  # 3rd layer number of features
...
weights = {
    ...
    'h3': tf.get_variable("W3", shape=[n_hidden_2, n_hidden_3],
                          initializer=tf.contrib.layers.xavier_initializer(uniform=False)),
    'out': tf.get_variable("W4", shape=[n_hidden_3, n_classes],
...
biases = {
    ...
    'out': tf.Variable(tf.constant(0.1, shape=[n_classes]), name="B4")
}
# tf Graph input
...
def multilayer_perceptron(x, num_hidden_layers, weights,
    ...
    layer_2 = tf.nn.dropout(
        layer_2, tf.constant(keep_prob), name="layer_2")
    layer_3 = tf.add(tf.matmul(layer_2, weights['h3']), biases[
                     'b3'], name="layer_3")
    layer_3 = activate(layer_3, activation, tf_name="layer_3")
    layer_3 = tf.scalar_mul(multiplication_factor, layer_3)
    layer_3 = tf.nn.dropout(
        layer_3, tf.constant(keep_prob), name="layer_3")
    # Output layer
    out_layer = tf.add(tf.matmul(layer_3, weights['out']), biases[
                       'out'], name="out_layer")
    return out_layer
...
print "copyright is not mine but the writer is :"
