...
index =  df_persons.Name.str.contains('hillary')
person_id = df_persons[].index.values # identificadores
...
df_emails.columns
#df_emails.groupby('SenderPersonId').SenderPersonId.count()
df_emails.MetadataTo.value_counts().head(10)
...
def cleanEmailText(text):
    ...
    text = re.sub(r"/[a-zA-Z]*[:\/\/]*[A-Za-z0-9\-_]+\.+[A-Za-z0-9\.\/%&=\?\-_]+/i", "", text) # Re 
    clndoc = ''
