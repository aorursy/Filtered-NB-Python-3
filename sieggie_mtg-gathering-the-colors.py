...
#    I think this is dirty data.  I'm not a MTG expert so maybe there is some rule I'm not aware of.
#    For example, Serra Angel needs white mana to cast and should count as white for Protection From White.
#    For example, Counterspell seems to me like the typical blue spell.
...
Counter(mtg['name']).keys()




countlist = Counter(mtg['name']).values()




df= pd.DataFrame(Counter(mtg['name']).keys().v,Counter(mtg['name']).values())
...
### Color distribution over time (releases).
### Note: hover over the interactive chart for counts.
### In the middle of 2007, Wizards of the Coast changed its strategy.
### It started to release more frequently with less cards per release.
...
### Let's focus on the creatures now.
creatures = mtg.loc[
...
### Mana distribution
### X-axis is a little weird.  Most start with 1 mana.  Exception is Colorless, which start at 0.
### There should not be a gap at 0.5. 
...
g = gg.ggplot(gg.aes(x='cmc', color='manaColors'), data=creatures)     + gg.geom_bar(size=1) + gg.facet_wrap('manaColors') 
print(g)
...
g = gg.ggplot(gg.aes(x='power', color='manaColors'), data=creatures)     + gg.geom_bar(size=1) + gg.facet_wrap('manaColors') 
print(g)
...
g = gg.ggplot(gg.aes(x='toughness', color='manaColors'), data=creatures)     + gg.geom_bar(size=1) + gg.facet_wrap('manaColors') 
print(g)




### Interactive Histogram of Creature Attributes by Color
### I could not get the colors to work correctly but ordering on hover text is alphabetical.
### Same as with Color Distribution Over Time plot.
data = []
...
### Radar chart factory code from http://matplotlib.org/examples/api/radar_chart.html
...
# We will look at how the distribution of card types varies by mana color.
# Each radar shows the distribution of cards into 5 types with every circle denoting 10%.
